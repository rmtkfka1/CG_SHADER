...
void viewport21_2::Render()
{
	...
	if (camera_bingle == false)
	{
		...
		if (camera_auto_rotate)
		{
			...
			glm::vec3 cameraPos = glm::vec3(camera_rotate_dx, 0, camera_rotate_dz); //--- 카메라 위치
		...
		else
		{
			...
			if (move_xy)
			{
				...
				v[0]->SetUniformMat4f("u_view",
					matrix::GetInstance()->GetCamera(glm::vec3(camera_dx, 0, camera_dz), glm::vec3(0, 0, 0)));
	...
	else
	{
		...
		glm::mat4 view = glm::mat4(1.0f);
		glm::vec3 cameraPos = glm::vec3(camera_dx, 0, camera_dz); //--- 카메라 위치
...
void viewport21_2::animation_1() //팔벌리기
{
	if (!anim_1)
	{
		return;
...
void viewport21_2::animation_2() //팔오므리기
{
